...
class HUMUSNet(nn.Module):#整个网络，所有的cascades
    """
    ...
    def forward(self, masked_kspace: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
        ...
        for i, cascade in enumerate(self.cascades):
            if self.use_checkpoint:
                kspace_pred = checkpoint.checkpoint(cascade, kspace_pred, masked_kspace, mask, sens_maps)#每个cascade保存一次权重
            else:
                kspace_pred = cascade(kspace_pred, masked_kspace, mask, sens_maps)
        
        #ASR, 只取中间的slice，故kspace_pred的维度1是slice，维度0应该为batch，维度2为线圈维度   
        kspace_pred = torch.chunk(kspace_pred, self.num_adj_slices, dim=1)[center_slice]
        
        #rss:将多个线圈的图合成一个图，维度1为线圈维度；整个网络的最后一步
        # complex_abs：计算复数绝对值，最后一个维度为2是复数维度；
        # ifft2c：逆傅里叶变换，最后一个维度为2是复数维度，不改变维度
        #ifft2c+complex_abs：将kspace变为图片
...
class VarNetBlock(nn.Module):#一个cascade
    """
    ...
    def sens_expand(self, x: torch.Tensor, sens_maps: torch.Tensor) -> torch.Tensor:
        #输入x:图像域中的，整张图片，无线圈维度但最后一维维度是2,(b, slice, h, w, 2)
        # c:num_slice * num_coil
        _, c, _, _, _ = sens_maps.shape
        # x.repeat_interleave:在维度1上重复线圈个数次
        # complex_mul:俩复数矩阵相乘，代表Si * x ,最后一维是复数维度
        #输出x：频率域，(b, slice*coil, h, w, 2)
        return fastmri.fft2c(fastmri.complex_mul(x.repeat_interleave(c // self.num_adj_slices, dim=1), sens_maps))

    def sens_reduce(self, x: torch.Tensor, sens_maps: torch.Tensor) -> torch.Tensor:#输入x(b, slice * coil, h, w, 2)
        #输入x:频率域中的，有线圈维度，c=slice * coil
        b, c, h, w, _ = x.shape
        #x:图像域中
        x = fastmri.ifft2c(x)
        #在num_coil维度上相加
        #输出x：图像域，无线圈维度(b, slice, h, w, 2)
        return fastmri.complex_mul(x, fastmri.complex_conj(sens_maps)).view(b, self.num_adj_slices, c // self.num_adj_slices, h, w, 2).sum(
            dim=2, keepdim=False
        )
    @staticmethod
    def complex_to_chan_dim(x: torch.Tensor) -> torch.Tensor:#复数维度加到channel
        b, c, h, w, two = x.shape
        assert two == 2
        return x.permute(0, 4, 1, 2, 3).reshape(b, 2 * c, h, w)

    @staticmethod
    def chan_complex_to_last_dim(x: torch.Tensor) -> torch.Tensor:#将复数维度提取出来加到最后一维
        b, c2, h, w = x.shape
        assert c2 % 2 == 0
        c = c2 // 2
    ...
    def apply_model(self, x):#x:图像域，(b, slice, h, w, 2)
    ...
    def forward(
        self,
        current_kspace: torch.Tensor,#(B, slice * coil, H, W, 2)
        ref_kspace: torch.Tensor,
        mask: torch.Tensor,
        sens_maps: torch.Tensor,#Si矩阵,(b, slice * coil, h, w, 2)
    ) -> torch.Tensor:       
        zero = torch.zeros(1, 1, 1, 1, 1).to(current_kspace)#设备转移
...
class SensitivityModel(nn.Module):
    ...
    def divide_root_sum_of_squares(self, x: torch.Tensor) -> torch.Tensor:
        return x / fastmri.rss_complex(x, dim=1).unsqueeze(-1).unsqueeze(1)#在线圈维度上计算rss
